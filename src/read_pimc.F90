...
subroutine read_pimc(sys,pimc,in_file)
    ...
    ! 0
    ! Do we use a restart file (currently does nothing)
    ! n
    ...
    ! Action type (0=Primitive Action, 1=Takahashi-Imada Action, 2=Chin Action)
    ! 2
...
8   format(a1)
    ...
    !pimc%free%beta_step = pimc%free%beta_step/dble((pimc%NumBlocks*(pimc%StepsPerBlock-pimc%BlocksToEquil))-1)
    pimc%free%beta_step = pimc%free%beta_step/dble(pimc%NumBlocks)
