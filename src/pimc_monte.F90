...
! main loop for path integral Monte Carlo
    ...
    subroutine pimc_monte(sys,pimc,Beads,OldBeads, out_dir,binning_in)
        ...
        do iblock=1,pimc%NumBlocks
            ...
            if (iter.eq.pimc%BlocksToEquil+1) then
                equil=.FALSE.
            endif
                ...
                do iatom=1,atom_pass
                    ...
                    enddo
                enddo
            enddo  ! end of the step
#ifdef FREE_ENERGY
                if(equil) then
                else
                    !If reversible scaling update the temperature at the end of each step
                    if(pimc%free%free_type==2.and. pimc%doFree==1) then
                        call reversibleScalingStep(free,pimc,iblock,dble(num_moves*atom_pass*(pimc%StepsPerBlock-pimc%BlocksToEquil)))
                        call tempStep(pimc)
                    endif
                endif
#endif      
            ...
            if(equil) then
                ...
                if(pimc%doSample==1) then
#endif
                ! calculate average energy and acceptance probability for each block and write them out
...
#ifdef FREE_ENERGY
                if(pimc%free%free_type==2.and. pimc%doFree==1) then
                    call writeIntInternal(free,pimc,B_init,dble(iblock))
                    !write(*,*) "Block: ", iblock, 'Final Temperature', pimc%Temperature
                endif
#endif
